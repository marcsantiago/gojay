...
func TestDecoderStringNullWithDecoder(t *testing.T) {
	testCases := []struct {
		name                 string
		json                 string
		expectedResult       string
		...
		{
			name:                 "utf8-code-point-err disabled",
			json:                 `"\u2Z70"`,
			expectedResult:       `�Z70`,
			disableStrictUnicode: true,
			err:                  false,
		},
		{
			name:           "utf16-surrogate",
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			str := (*string)(nil)
			decoder := NewDecoder(bytes.NewReader([]byte(testCase.json)))
			if testCase.disableStrictUnicode {
				decoder.DisableStrictUnicode()
			}
